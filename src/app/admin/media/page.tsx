...
export default async function AdminMediaPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="hv-panel-sci flex flex-col gap-4 p-4 sm:p-6 sm:flex-row sm:items-end sm:justify-between relative overflow-hidden">
        <div className="absolute left-0 top-0 h-10 w-10 border-l-2 border-t-2 border-accent/60 pointer-events-none" />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className={orphanCount > 0 ? "border border-amber-300/35 bg-accent/10 px-2 py-0.5 font-mono text-xs text-amber-100" : "hv-chip-sci"}>{orphanCount} 张未引用</span>
        </div>
      </header>
              ...
              <div key={item.url} className="hv-panel-sci overflow-hidden p-0">
                <div className="relative aspect-video overflow-hidden bg-black/40">
                  ...
                  <div className="absolute right-2 top-2 flex gap-1.5">
                    {refCount === 0 ? (
                      <span className="dark-locked border border-amber-200/50 bg-accent/90 px-2 py-0.5 text-[10px] font-medium text-white font-mono uppercase">ORPHAN</span>
                    ) : (
                      <span className="dark-locked border border-emerald-200/50 bg-accent/90 px-2 py-0.5 text-[10px] font-medium text-white font-mono uppercase">REF ×{refCount}</span>
                    )}
                  </div>
                </div>
                <div className="flex flex-col gap-2 p-3 text-xs">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    <a href={item.url} target="_blank" rel="noreferrer" className="hv-action min-h-0 px-2 py-0.5 text-[11px] font-mono uppercase">
                    ...
                    <form
                      ...
                      className="ml-auto"
                    >
                      <button type="submit" className="inline-flex items-center gap-1 border border-red-400/35 bg-red-500/10 px-2 py-0.5 text-[11px] text-red-200 transition hover:border-red-300 hover:bg-red-500/15 font-mono uppercase">
